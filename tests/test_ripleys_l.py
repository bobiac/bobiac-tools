"""Tests for ``ripleys_l`` (Ripley's L function with Monte Carlo envelope)."""
...
from bobiac_tools._ripleys_l import ripleys_l
...
def test_returns_three_arrays():
    mask = make_mask()
    result = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=0)
    assert len(result) == 3


def test_r_values_shape():
    mask = make_mask()
    r_vals, L_obs, L_sims = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=0)
...
def test_L_observed_shape_matches_r_values():
    mask = make_mask()
    r_vals, L_obs, _ = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=0)
    assert L_obs.shape == r_vals.shape


def test_L_sims_shape():
    mask = make_mask()
    r_vals, _, L_sims = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=8, seed=0)
...
def test_r_values_start_at_zero_by_default():
    mask = make_mask()
    r_vals, *_ = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=0)
...
def test_same_seed_gives_same_result():
    mask = make_mask()
    r1, obs1, sims1 = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=9)
    r2, obs2, sims2 = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=9)
...
def test_different_seeds_give_different_sims():
    mask = make_mask()
    _, _, sims1 = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=1)
    _, _, sims2 = ripleys_l(csr_spots(), mask, cell_id=1, n_repeats=5, seed=2)

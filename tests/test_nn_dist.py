"""Tests for ``nn_dist`` (nearest-neighbor distance clustering test)."""
...
from bobiac_tools._nn_dist import nn_dist
...
def test_returns_four_floats():
    mask = make_mask()
    result = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=20, seed=0)
    assert len(result) == 4
...
def test_spot_dist_avg_is_positive():
    mask = make_mask()
    spot_dist_avg, *_ = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=20, seed=0)
    assert spot_dist_avg > 0.0


def test_rnd_dist_avg_is_positive():
    mask = make_mask()
    _, rnd_dist_avg, *_ = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=20, seed=0)
    assert rnd_dist_avg > 0.0


def test_p_value_in_unit_interval():
    mask = make_mask()
    *_, p_value = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=20, seed=0)
    assert 0.0 <= p_value <= 1.0


def test_clark_evans_equals_ratio():
    mask = make_mask()
    spot_d, rnd_d, ce, _ = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=20, seed=0)
...
def test_clustered_spots_have_low_p_value():
    ...
    mask = make_mask()
    *_, p_value = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=100, seed=0)
    assert p_value < 0.05
...
def test_clustered_spots_have_ce_below_one():
    ...
    mask = make_mask()
    _sd, _rd, ce, _ = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=100, seed=0)
    assert ce < 1.0
...
def test_dispersed_spots_have_ce_above_one():
    ...
    mask = make_mask()
    _sd, _rd, ce, _ = nn_dist(dispersed_spots(), mask, cell_id=1, n_repeats=100, seed=0)
    assert ce > 1.0
...
def test_same_seed_gives_same_result():
    mask = make_mask()
    r1 = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=30, seed=7)
    r2 = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=30, seed=7)
    assert r1 == r2


def test_different_seeds_give_different_rnd_avg():
    mask = make_mask()
    _, rnd1, *_ = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=30, seed=1)
    _, rnd2, *_ = nn_dist(clustered_spots(), mask, cell_id=1, n_repeats=30, seed=2)
